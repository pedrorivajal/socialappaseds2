...
router.put("/:id/follow", async (req, res) => {
  if (req.body.userId !== req.params.id) {
    ...
    res.status(403).json("tu ne peux pas te suivre");
  }
});
  ...
  if (req.body.userId !== req.params.id) {
    try {
      ...
      if (user.followers.includes(req.body.userId)) {
        ...
        res.status(200).json("l'utilisateur n'a plus été suivi");
      } else {
    ...
    res.status(403).json("vous ne pouvez pas vous désabonner");
  }
});
